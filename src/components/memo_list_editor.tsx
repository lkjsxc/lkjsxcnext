  ...
  const [editedMemoContent, setEditedMemoContent] = useState('');
    ...
    const handleUpdateMemo = async (id: string, title: string, content: string, isPublic?: boolean) => {
      try {
        ...
        // We don't clear editing state on auto-save, only on manual save or cancel
        // if (editingMemoId === id) {
        //   handleCancelEdit();
        // }
      } catch (err) {
    ...
    const handleEditClick = (memo: { id: string; title: string; content: string | null }) => {
      ...
      // No need to debounce here, this is just setting initial state
    };
    ...
    const handleCancelEdit = () => {
      ...
      // Cancel any pending debounced saves when cancelling edit
      debouncedSetEditedMemoTitle.cancel();
      debouncedSetEditedMemoContent.cancel();
    };
  
    // Debounced state setters that trigger save
    const debouncedSetEditedMemoTitle = useCallback(
      debounce((title: string, memoId: string, currentContent: string, isPublic?: boolean) => {
        setEditedMemoTitle(title);
        if (memoId) {
          handleUpdateMemo(memoId, title, currentContent, isPublic);
        }
      }, 1000), // 1000ms debounce delay
      [handleUpdateMemo] // Dependency array for useCallback
    );
  
    const debouncedSetEditedMemoContent = useCallback(
      debounce((content: string, memoId: string, currentTitle: string, isPublic?: boolean) => {
        setEditedMemoContent(content);
        if (memoId) {
          handleUpdateMemo(memoId, currentTitle, content, isPublic);
        }
      }, 1000), // 1000ms debounce delay
      [handleUpdateMemo] // Dependency array for useCallback
    );
          ...
          <ul className="space-y-4">
            {memo.map((memo) => (
              <MemoItem
                ...
                onUpdateMemo={handleUpdateMemo} // Keep original for manual save
                ...
                setEditedMemoTitle={(title) => {
                  // Call the debounced setter
                  debouncedSetEditedMemoTitle(title, memo.id, editedMemoContent, memo.isPublic);
                }} // Pass down debounced state setters
                setEditedMemoContent={(content) => {
                  // Call the debounced setter
                  debouncedSetEditedMemoContent(content, memo.id, editedMemoTitle, memo.isPublic);
                }} // Pass down debounced state setters
              />
            ))}
          </ul>
